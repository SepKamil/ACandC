...
# ongoing_event types are only defined when type checking
_OngoingEventT = Any
_ParticipantT = Any
if TYPE_CHECKING:
    from .ongoing_event import OngoingEvent
    from .participant import Participant

    _OngoingEventT = OngoingEvent
    _ParticipantT = Participant


class EffectReference:
    def __init__(self, participant_id, effect_id):
        self.participant_id = participant_id
        self.effect_id = effect_id

    @classmethod
    def from_effect(cls, effect):
        return cls(effect.participant.id, effect.id)

    @classmethod
    def from_dict(cls, d):
        return cls(d["participant_id"], d["effect_id"])

    def to_dict(self):
        return {"participant_id": self.participant_id, "effect_id": self.effect_id}


class Effect:
    def __init__(
        self,
        ongoing_event: Optional[_OngoingEventT],
        participant: Optional[_ParticipantT],
        id: str,
        name: str,
        duration: int,
        remaining: int,
    ...
    def new(
        ...
        if isinstance(effect_args, str):
            if (participant and participant.type == ParticipantType.PLAYER) or character:
                effect_args = argparse(effect_args, participant.character or character)
            else:
    ...
    @classmethod
    def from_dict(cls, raw, ongoing_event, participant):
        ...
        return cls(ongoing_event, participant, children=children, parent=parent, **raw)
    ...
    def _duration_cmp(self):
        ...
        if self.participant is None or self.ongoing_event is None:
    ...
    def _effect_from_reference(self, e: EffectReference):
        if self.ongoing_event is None:
            return None
        participant = self.ongoing_event.participant_by_id(e.participant_id)
        if participant is None:
            return None
        effect = participant.effect_by_id(e.effect_id)
        if effect is None:
            return None
        return effect
    ...
    def remove(self, removed=None):
        ...
        if self.participant is not None:
            self.participant.remove_effect(self)
